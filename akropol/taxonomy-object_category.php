<? get_header(); ?>
<?$objects_category = $wp_query->query['object_category'];?>
<?//=$objects_category?>
<?
$true_taxonomy = 'object_category'; // таксономия - регион
$myterm = get_term_by('slug' , $objects_category, $true_taxonomy);
...
<? include get_template_directory() . "/include/filter.php"; ?>
  <div class="main" role="main">
    <div id="content" class="content full content-objects">
      <div class="container">
        <div class="row">
					<div class="col-md-12">
						<div class="block-heading"><h4><span class="heading-icon"><i class="fa fa-bars" aria-hidden="true"></i></span><?=$description?></h4></div>
					</div>
          <div class="navObjects">
            <div class="breadcrumbs" itemscope="" itemtype="http://schema.org/BreadcrumbList">
              ...
              //$myterm = get_term_by('slug' , $objects_category, $true_taxonomy);
              //print_r($myterm);
              $term_id = $myterm->term_id;// ID дочернего термина
              $true_terms = array();
              while( $parent_id = wp_get_term_taxonomy_parent_id( $term_id, $true_taxonomy ) ){
                array_push($true_terms,  $parent_id);
                $term_id = $parent_id;
                //echo $parent_id." ";
              }
              if(!empty($true_terms)) {
                array_push($true_terms,  $myterm->term_id);
                //print_r($true_terms);
                $term_array = trim(implode(',', (array)$true_terms), ' ,');
                $neworderterms = get_terms($true_taxonomy, 'orderby=none&include=' . $term_array);
                foreach ($neworderterms as $key => $orderterm) {
                  if(!empty($neworderterms[$key+1]))
                    ...
                    echo '<span class="current">' . $orderterm->name . '</span>';
                }
              }
              else {
                $neworderterms = get_terms($true_taxonomy, 'orderby=none&include=' . $myterm->term_id);
            ...
            <div class="newCategory">
              <?php
              $taxonomyName = "object_category";
              $myterm = get_term_by('slug' , $objects_category, $taxonomyName);
              ...
              foreach ($termchildren as $child) {
                ...
                if ($term->parent == $termID && $term->count != 0)
