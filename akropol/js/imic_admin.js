...
//SELECTED TEMPLATE BASED META BOX DISPLAY
           ...
           var metaID = jQuery('#page_template').val().slice(0, -4);
            ...
            { 
				...
				if((metaID=='template-property-listing')||(metaID=='template-contact')) {
					jQuery('#imic_banner_type option:gt(2)').show();
				}
				else{
					jQuery('#imic_banner_type option:gt(2)').hide();	
				}
                jQuery("#cr_upload_meta_box2").show();               
                jQuery("#cr_upload_meta_box").show();               
            })
        }
    }
    show_hide_metabox();
   ...
   function checkPostFormat(radio_val) {
    ...
    function bannerTypeOptions() {
       ...
       var $sliderImage = jQuery('ul[data-field_id=imic_slider_image]').parent().parent();
